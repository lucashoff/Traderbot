...
function getMaxValueToBuy(low, high){
   low = parseFloat(low);
   high = parseFloat(high);
   var diff = (high - low);
   var maxValue = low + parseFloat(diff * 0.4); // calcula valor limite de compra com 40% do spread
...
setInterval(() => 
   infoApi.ticker((response) => {
      ...
      var limitValueToBuy = getMaxValueToBuy(response.ticker.low, response.ticker.high);
      console.log('Valor limite de compra:'+limitValueToBuy);
      if(response.ticker.sell <= limitValueToBuy){
         ...
         console.log("Valor atual da moeda maior que o limite estipulado.");
      }
   }),
